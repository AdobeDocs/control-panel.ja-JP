...
>title="SSL 証明書の更新"
>abstract="SSL 証明書を更新するには、CSR を生成し、サブドメインの SSL 証明書を購入して、証明書バンドルをインストールする必要があります。この操作は、証明書をAdobeに委任するのではなく、証明書を手動で管理するように選択した場合にのみ必要です。 "

>[!NOTE]
>
>サブドメインの SSL 証明書の更新は、証明書を自分で管理することを選択した場合にのみ必要です。このプロセスをAdobeに委任するのではなく、お客様自身で証明書を管理する必要があります。 Adobeは証明書を自動的に作成し、有効期限が切れる前に毎年更新するので、サブドメインの SSL 証明書の管理をAdobeにデリゲートすることを強くお勧めします。 [SSL 証明書管理の詳細を説明します](monitoring-ssl-certificates.md#management)
